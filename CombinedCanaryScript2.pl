...
#	system ("Rscript CombinedCanaryScript3.r $TreeArray[$x] $_.$CompareClades $_");
	print "Rscript CanarySoIScript.r $BaseTree $TreeArray[$x] $CompareClades $_.$CompareClades $_ $Outgroup > $_.CanaryAssessment.txt\n";
	system ("Rscript CanarySoIScript.r $BaseTree $TreeArray[$x] $CompareClades $_.$CompareClades $_ $Outgroup > $_.CanaryAssessment.txt");
	push (@CanaryAssess, "$_.CanaryAssessment.txt");
	$x++;
}
...
#			print "@reader\n";
			if (grep (/"Novel"/, @reader) && grep (/"Non-Isomorphic"/, @reader)){
					print "$InFile \t Novel \t Non-Isomorphic \t Ambiguous Sequence\n";
					}
			elsif (grep (/"Novel"/, @reader) && grep (/"Isomorphic"/, @reader)){
				print "$InFile \t Novel \t Isomorphic \t Canary Sequence\n";
				}
			elsif (grep (/"Non-Novel"/, @reader) && grep (/"Non-Isomorphic"/, @reader)){
				print "$InFile \t Non-novel \t Non-Isomorphic \t Ambiguous Sequence\n";
				}
			elsif (grep (/"Non-Novel"/, @reader) && grep (/"Isomorphic"/, @reader)){
				print "$InFile \t Non-novel \t Isomorphic \t Ambiguous Sequence\n";
				}			
#			print scalar @reader, "\n";
			undef @reader;
}